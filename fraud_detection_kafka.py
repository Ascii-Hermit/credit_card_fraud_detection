...
# Sending both 'isFraud' and 'prediction' values to Kafka
for row in predictions.select("isFraud", "prediction").collect():
    true_label = int(row.isFraud)
    predicted_value = int(row.prediction)
    prediction_message = {'is_fraud': true_label, 'predicted_value': predicted_value}
    producer.send('fraud_predictions', value=prediction_message)
    print(f"Sent prediction to Kafka: {prediction_message}")
    time.sleep(1)
